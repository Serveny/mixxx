...
def read_doctree(doc_rel_path: str):
    doctree_path = manual_dir / "build" / ".doctrees" / f"{doc_rel_path}.doctree"
...
def create_group_control_linking(
    ...
    groups_and_count = [replace_dynamics(group.name) for group in groups]
    ...
    # dynamic groups
    dynamics = [
        f"[key: `{groups_and_count[i][0]}`]: {prefix}{group_var(groups[i].name)};"
...
def get_group(name: str, groups: list[Group]) -> Group:
    try:
        ...
        raise Exception(
            ...
            Add .. mixxx:cogroupdesc:: {name} and the description below to mixxx_controls.rst"""
        )
...
def export_ts_types(
    ...
    rw_groups = [get_group(name, group_info) for name in extract_groups(rw_controls)]
    ...
    ro_groups = [get_group(name, group_info) for name in extract_groups(ro_controls)]
